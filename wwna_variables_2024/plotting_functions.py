...
def save_and_close(path, step, dpi=FIGURE_DPI):
    """Save figure to processed_data/step{step}/path and close it."""
    full_path = f"processed_data/step{step}/{path}"
    plt.tight_layout()
...
def plot_facilities_map(
    num_parameters_per_facility: Dict[str, int],
    legend_label: str,
    label_threshold: int,
) -> None:
    """
    ...
    Args:
        num_parameters_per_facility: Dictionary mapping
    ...
    if facilities_gdf.crs != ca_counties.crs:
        ca_counties = (
            ca_counties.set_crs(facilities_gdf.crs)
        ...
        facilities_gdf = (
            facilities_gdf
            if ca_counties.crs is None
            else facilities_gdf.to_crs(ca_counties.crs)
        )

    # Create plot
    fig, ax = setup_figure(figsize=(8, 5))
    ca_counties.plot(ax=ax, color="lightgray")
    facilities_gdf["num_parameters"] = facilities_gdf["NPDES_CODE"].map(
    ...
    # Add facility labels
    top_facilities = (
        facilities_gdf[facilities_gdf["num_parameters"] >= label_threshold]
    ...
    for idx, (_, row) in enumerate(top_facilities_sorted.iterrows()):
        if (
            ...
            and pd.notna(row.geometry.x)
            and pd.notna(row.geometry.y)
        ):
...
def plot_facilities_summary(
    num_parameters_per_facility: Dict[str, int],
    title: str = "Facilities with Exceedances",
) -> None:
...
def plot_future_limits_summary(df_sorted: pd.DataFrame) -> None:
    ...
    save_and_close("figures_py/facilities_with_future_limits_efficient.png", 4)
...
def generate_facility_plots(
    facilities_list: pd.DataFrame, limits_2023: pd.DataFrame
) -> None:
